...
describe("MacroCalculator", () => {
  ...
  test('Calcula o TMB corretamente para o gênero masculino', () => {
    const tmb = MacroCalculator.calcularTMB(70, 1.75, 25, generos.MASCULINO);
    expect(tmb).toBeCloseTo(1673.75);
  });

  test('Calcula o TMB corretamente para o gênero feminino', () => {
    const tmb = MacroCalculator.calcularTMB(70, 1.75, 25, generos.FEMININO);
    expect(tmb).toBeCloseTo(1507.75);
  });

  test('Exibe erro para valores de peso não numéricos', () => {
    expect(() => {
      MacroCalculator.calcularTMB("70", 1.75, 25, generos.MASCULINO);
  ...
  test('Calcula o TDEE corretamente para uma pessoa sedentaria', () => {
    const tmb = 2000;
    const tdee = MacroCalculator.calcularTDEE(tmb, atividade.SEDENTARIO);
    expect(tdee).toBe(2400);
  });

  test('Calcula o TDEE corretamente para uma pessoa muito ativa', () => {
    const tmb = 2000;
    const tdee = MacroCalculator.calcularTDEE(tmb, atividade.MUITO_ATIVO);
  ...
  test('Exibe erro para valores de atividade não especificados', () => {
    const tmb = 2000;
    expect(() => {
      MacroCalculator.calcularTDEE(tmb, "EXTREMAMENTE_ATIVO");
  ...
  test('Calcula corretamente os macros para o bulking', () => {
    ...
    const expectedCalories = MacroCalculator.calcularTDEE(MacroCalculator.calcularTMB(peso, altura, idade, generos.MASCULINO), atividade.MODERADAMENTE_ATIVO) + 250;
  ...
  test('Calcula corretamente os macros para o cutting', () => {
    ...
    const expectedCalories = MacroCalculator.calcularTDEE(MacroCalculator.calcularTMB(peso, altura, idade, generos.FEMININO), atividade.POUCO_ATIVO) -500;
    expect(macro.Proteinas).toBe(Math.round((expectedCalories * macros.PROTEINA) / 4));
    expect(macro.Carboidratos).toBe(Math.round((expectedCalories * macros.CARBOIDRATO) / 4));
    expect(macro.Gorduras).toBe(Math.round((expectedCalories * macros.GORDURA) / 9));
  });
  ...
  test('Calcula corretamente os macros para manter o peso', () => {
    ...
    const expectedCalories = MacroCalculator.calcularTDEE(MacroCalculator.calcularTMB(peso, altura, idade, generos.FEMININO), atividade.POUCO_ATIVO);
    expect(macro.Proteinas).toBe(Math.round((expectedCalories * macros.PROTEINA) / 4));
    expect(macro.Carboidratos).toBe(Math.round((expectedCalories * macros.CARBOIDRATO) / 4));
    expect(macro.Gorduras).toBe(Math.round((expectedCalories * macros.GORDURA) / 9));
  });
